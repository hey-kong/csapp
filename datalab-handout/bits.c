 ...
 */
 // 0x38 = 111000
 // 等价于将 0xFFFFFFFF 的 [31, highbit+1] 与 [lowbit-1, 0] 位置为 0
 // 0xFFFFFFFF 左移 lowbit 位
 // 0xFFFFFFFF 左移 highbit+1 位再取反
 // 两数进行与操作
...
int isLess(int x, int y) {
  ...
  // x 和 y 符号相同
  int sign_2 = !((x^y)>>31)&1;
  return sign_1 | (sign_2&((x+(~y+1))>>31)&1);
}
/* 
...
unsigned float_i2f(int x) {
  ...
  x = x << (31-n);
  frac = (x >> 8) & frac_mask;
  // int转换为float要损失8位的精度，判断最后8位是否要进位
  x = x & 0xff;
  round = (x > 128) || ((x == 128) && (frac & 1));
  frac += round;
  // 若原frac等于0x007fffff，frac加1之后需要重新设置exp和frac
  if (frac >> 23) {
    exp += 0x800000;
    frac = frac & frac_mask;
  }
